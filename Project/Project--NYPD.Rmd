...
# function: calculate the bounds of a numeric variable; need to input a numeric vector
...
# Histogram --- Not excludes any values
...
# Observed time: male VS female (excludes large numbers)
...
# Observed time: male VS female (excludes outliers)
...
# And then we make 4 separate barplots -- 1 for each category, showing the distribution of the specific crime descriptions inside each category
...
```{r}
# FRISKED_FLAG and SEARCHED_FLAG into dataframe
...
# Observed time: only frisked VS only searched VS both VS none (excludes outliers)
bound(NYPD$OBSERVED_DURATION_MINUTES)
...
# Stop time: only frisked VS only searched VS both VS none (excludes outliers)
bound(NYPD$STOP_DURATION_MINUTES)
